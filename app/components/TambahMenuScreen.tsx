...
function menuFormReducer(state: MenuFormState, action: MenuFormAction): MenuFormState {
  switch (action.type) {
    case "SET_NAME": return { ...state, name: action.payload };
    case "SET_PRICE": return { ...state, price: action.payload };
    case "SET_CATEGORY_ID": return { ...state, categoryId: action.payload };
...
function modifierDraftReducer(state: ModifierDraftState, action: ModifierDraftAction): ModifierDraftState {
  switch (action.type) {
    case "SET_MOD_NAME":
    ...
    case "ADD_MODIFIER": {
      ...
      return {
        ...
        modifiers: [
          ...state.modifiers,
          { id: crypto.randomUUID(), name: state.modName.trim(), priceDelta: delta },
        ],
        modName: "",
        modPrice: "",
        modError: "",
    ...
    case "REMOVE_MODIFIER":
      return { ...state, modifiers: state.modifiers.filter((m) => m.id !== action.payload) };
    default:
      return state;
  }
}
...
function submissionReducer(state: SubmissionState, action: SubmissionAction): SubmissionState {
  switch (action.type) {
    ...
    case "SET_SUBMITTING": return { ...state, isSubmitting: true, error: "" };
    case "SET_SUCCESS": return { ...state, success: true, isSubmitting: false };
    ...
    <div className="bg-surface-container-lowest rounded-2xl p-5 shadow-ambient border border-outline-variant">
      ...
      <div className="mb-4">
        ...
        <input
          ...
          type="text"
          value={form.name}
          onChange={(e) => { dispatchForm({ type: "SET_NAME", payload: e.target.value }); onClearError(); }}
      ...
      <div className="mb-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            inputMode="numeric"
            value={form.price}
            onChange={(e) => { dispatchForm({ type: "SET_PRICE", payload: formatPriceInput(e.target.value) }); onClearError(); }}
      ...
      <div className="mb-4">
        ...
        {categoriesLoading ? (
          ...
          <select
            ...
            onChange={(e) => dispatchForm({ type: "SET_CATEGORY_ID", payload: e.target.value })}
  ...
  return (
    <div className="bg-surface-container-lowest rounded-2xl p-5 shadow-ambient border border-outline-variant">
      <h2 className="text-sm font-bold text-on-surface mb-1 flex items-center gap-2">
        ...
        <span className="ml-auto text-xs text-outline font-normal">{draft.modifiers.length} ditambahkan</span>
      </h2>
          ...
          {draft.modifiers.map((mod) => (
            ...
            >
              ...
              <button
                ...
                onClick={() => dispatchDraft({ type: "REMOVE_MODIFIER", payload: mod.id })}
      ...
      <div className="bg-surface-container-low rounded-xl p-3 border border-surface-container-high">
        ...
        <div className="flex gap-2 mb-2">
          <input
            ...
            value={draft.modName}
            onChange={(e) => dispatchDraft({ type: "SET_MOD_NAME", payload: e.target.value })}
            onKeyDown={(e) => e.key === "Enter" && dispatchDraft({ type: "ADD_MODIFIER" })}
          ...
          <div className="relative w-32 flex-shrink-0">
            ...
            <input
              ...
              onChange={(e) => dispatchDraft({ type: "SET_MOD_PRICE", payload: formatPriceInput(e.target.value) })}
              onKeyDown={(e) => e.key === "Enter" && dispatchDraft({ type: "ADD_MODIFIER" })}
              placeholder="0"
        ...
        {draft.modError && (
          <p className="text-error text-xs mb-2 font-semibold">{draft.modError}</p>
        )}
        <button
          type="button"
          id="btn-add-modifier"
...
export default function TambahMenuScreen() {
  ...
  const [draft, dispatchDraft] = useReducer(modifierDraftReducer, initialModifierDraft);
  const [submission, dispatchSubmission] = useReducer(submissionReducer, initialSubmission);
  ...
  const handleSave = async () => {
    ...
    if (!form.name.trim()) {
      dispatchSubmission({ type: "SET_ERROR", payload: "Nama menu tidak boleh kosong" });
      return;
    }
    const parsedPrice = parseInt(form.price.replace(/\D/g, ""), 10);
    if (!parsedPrice || parsedPrice <= 0) {
      dispatchSubmission({ type: "SET_ERROR", payload: "Harga harus lebih dari 0" });
      return;
    }

    dispatchSubmission({ type: "SET_SUBMITTING" });
    try {
      let finalModifiers = [...draft.modifiers];

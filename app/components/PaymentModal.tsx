...
export default function PaymentModal({
  ...
  order: Order;
  onConfirm: (payment: { paymentMethod: "cash" | "qris"; amountPaid: number; changeGiven: number }) => void;
  onClose: () => void;
}) {
      ...
      <div className="relative flex items-center justify-center h-full p-4 pointer-events-none">
        ...
        <div className="bg-primary px-6 py-5 rounded-t-2xl">
          <div className="flex items-center justify-between">
            ...
            <span className="text-on-primary/80 text-sm capitalize">{order.customerName}</span>
          </div>
        </div>
          ...
          {order.itemList.map((item) => (
            <div key={`${item.name}-${item.unitPrice}`} className="flex items-start justify-between text-sm">
              <div className="flex-1 min-w-0">
                ...
                {item.notes && (
                  <p className="text-xxs text-outline italic mt-0.5">{item.notes}</p>
                )}
              </div>
              <span className="text-on-surface-variant ml-4 flex-shrink-0">
            ...
            <div>
              ...
              {changeGiven > 0 && (
                <div className="mt-3 flex justify-between items-center bg-surface-container rounded-xl px-4 py-2.5">
                  <span className="text-sm font-semibold text-on-surface-variant">Kembalian</span>
                  <span className="text-lg font-bold text-on-surface">{formatRupiah(changeGiven)}</span>
                </div>
              )}
            </div>
          )}
